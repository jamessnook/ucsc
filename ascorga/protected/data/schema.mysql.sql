...
CREATE TABLE user (    -- AIS feed and creation by admins
    ...
    username     VARCHAR(63)  NOT NULL PRIMARY KEY,   --  cruzid if available
    div_data_id     VARCHAR(15) UNIQUE,   
    emplid       VARCHAR(63) UNIQUE,     -- AIS user id
    ...
    eff_date     DATE,                    -- when became active senate faculty
    created      DATETIME,                -- when imported from AIS
    modified     DATETIME,                -- when updated from AIS
    password     VARCHAR(127),            -- optional for local login
    salt         VARCHAR(127),            -- optional for local login
    modified_by  VARCHAR(63)              -- when updated from AIS
);
Alter table user add index (div_data_id);
-- ALTER TABLE user ADD COLUMN isCurrent BOOLEAN DEFAULT true;
